    ...
    const fetchDoctor = async () => {
      try {
        const res = await axios.get(
          `https://docmeet1.onrender.com/docmeet/admin/getDoctor/${id}`
        );
        const data = res.data?.data || res.data; // fix if wrapped in 'data'
        console.log(data);
        const timings = data.doctorTimmings || {
          doctorStart: "",
          doctorEnd: "",
        };
        const available = Array.isArray(data.doctorAvailableDays)
          ? data.doctorAvailableDays
          : [];
