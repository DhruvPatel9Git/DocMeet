...
function UserAppointmentsPage() {
  ...
  const fetchPrescriptions = (email) => {
    axios
      .get(`http://localhost:5001/docmeet/doctor/getPrescriptions/${email}`)
      .then((res) => {
          ...
          <table className="min-w-full table-auto border-collapse text-sm text-gray-700">
            <thead className="bg-indigo-100 text-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-center font-semibold">Prescription</th>
              </tr>
            </thead>
            <tbody>
                ...
                appointmentsAll.map((val, index) => (
                  <React.Fragment key={index}>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        {prescriptions[userData?.email] && prescriptions[userData?.email].length > 0 ? (
                    ...
                    {expandedRow === index && prescriptions[userData?.email] && prescriptions[userData?.email].length > 0 && (
                      <tr className="bg-indigo-50">
                        <td colSpan="6" className="px-6 py-4">
                          <div className="space-y-4">
                            <h3 className="text-lg font-semibold text-gray-700 mb-4">Prescriptions</h3>
                            {prescriptions[userData?.email].map((prescription, pIndex) => (
                              <div key={pIndex} className="bg-white p-4 rounded-lg border border-indigo-200">
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                  <div>
                                    <p className="text-sm text-gray-500">Patient Name</p>
                                    <p className="font-medium text-gray-700">{prescription.patientName}</p>
                                  </div>
                                  <div>
                                    <p className="text-sm text-gray-500">Patient Email</p>
                                    <p className="font-medium text-gray-700">{prescription.patientEmail}</p>
                                  </div>
                                </div>
                                <div className="mb-4">
                                  <p className="text-sm text-gray-500">Additional Info</p>
                                  <p className="font-medium text-gray-700">{prescription.additionalInfo}</p>
                                </div>
                                {prescription.prescriptionPhoto && (
                                  <div>
                                    <p className="text-sm text-gray-500 mb-2">Prescription Photo</p>
                                    <img 
                                      src={prescription.prescriptionPhoto.imgPath} 
                                      alt="Prescription" 
