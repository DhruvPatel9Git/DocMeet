...
function SignInPage() {
  ...
  const { userLoggedIn, setUserLoggedIn, logout, login } =
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const res = await axios.post(
        "https://docmeet1.onrender.com/docmeet/user/googlesignin",
        { token }
      );
      ...
      if (res.data.token) {
        ...
        alert(
          "Login failed. " + (res.data.msg || "No token returned from backend.")
        );
      }
    } catch (error) {
      ...
      if (error.code === "auth/popup-blocked") {
        ...
        alert("Sign-in request was cancelled.");
      } else if (error.response?.data?.msg) {
        alert("Login failed: " + error.response.data.msg);
      } else {
        alert(
          "Google Sign-In Error: " + (error.message || "Unknown error occurred")
        );
      }
    }
  };
  ...
  const handleSignInClick = () => {
    ...
    axios
      .post(`https://docmeet1.onrender.com/docmeet/user/signin`, signInData)
      .then((res) => {
      ...
      <div className="bg-white/20 backdrop-blur-md border-white/30 flex flex-col gap-2 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg">
        ...
        <div className="w-full text-right mt-1">
          <Link
            ...
            className="text-[#5D6BFF] text-sm underline"
          >
            Forgot Password?
          </Link>
        </div>
        ...
        <button
          ...
          className="bg-[#5D6BFF] text-white w-full py-2 !rounded-lg text-base mt-3"
        >
          SignIn
        </button>
        <button
        ...
        <p className="mt-3 text-center w-full">
          ...
          <Link to="/user/signup">
            <span className="text-[#5D6BFF] underline cursor-pointer">
              SignUp Here
            </span>
          </Link>
        </p>
