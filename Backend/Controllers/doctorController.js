...
const getAppointments = async (req, res) => {
  ...
  const getAppointmentsDetails = await appointmentModel.find({ doctorID: req.doctor._id, newDate: { $gt: date },monthNum : {$gte : monthNumCheck},slotDate: {$ne: fullDate} }).populate("doctorID").populate("userID")
...
const completedAppointment1 = async (req,res) => {
  const { userID, doctorID, slotTime, slotDay, slotDate, status,completedAppointment } = req.body  
  try {
    const updateResult = await appointmentModel.updateOne({
      userID: userID,
      doctorID: doctorID,
...
const getPrescriptionsByEmail = async (req, res) => {
  try {
    ...
    if (!prescriptions || prescriptions.length === 0) {
      return res.status(200).json({ msg: "No prescriptions found", prescriptions: [] });
    }

    res.status(200).json({ msg: "Prescriptions fetched successfully", prescriptions });
  } catch (error) {
...
module.exports = { filterData1,completedAppointment1,getAppointmentsPast,filterStatus,filterMonth, addPrescription, addMedicine, doctorDashboardName, getAppointments, updatestatuspostive, updatestatusnegative, getAppointmentsToday, getPrescriptionsByEmail };
